...
{
    // Accept connections on both listeners concurrently
    let (_, _) = tokio::join!(
        accept_connections(first, &reply),
        accept_connections(second, &reply),
    );
}

async fn accept_connections<T>(listener: TcpListener, reply: &T)
where
    T: Display + Send + Sync + 'static,
{
    loop {
